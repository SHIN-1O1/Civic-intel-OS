...
// Backend API URL - change to production URL when deployed
const API_BASE_URL = 'http://localhost:5000';

async function verifyComplaintWithGemini(location, description, category) {
    try {
        const response = await fetch(`${API_BASE_URL}/api/verify-complaint`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ location, description, category })
        });
...
async function handleComplaintSubmit(e) {
    ...
    // Stage 3: Confirmation (with context-aware AI analysis)
    if (APP_STATE.complaintStage === 3) {
        // Call AI to analyze user intent
        addMessageToChat(`<em>Analyzing your response...</em>`, 'bot');

        try {
            const verification = APP_STATE.complaintDraft.verification;
            const response = await fetch(`${API_BASE_URL}/api/analyze-confirmation`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userResponse: text,
            ...
            if (intentResult.intent === 'CONFIRM') {
                ...
                try {
                    const docRef = await addDoc(collection(db, "complaints"), {
                        ...
                        status: 'Pending',
                        severity: verification.severity || 'Medium',
                        summary: verification.summary || APP_STATE.complaintDraft.description.substring(0, 50) + "...",
                        aiVerified: true,
                ...
                // Update the verification with new severity/summary
                verification.severity = intentResult.updatedSeverity || verification.severity;
                verification.summary = intentResult.updatedSummary || verification.summary;
                APP_STATE.complaintDraft.verification = verification;

                // Also append the additional context to description
                APP_STATE.complaintDraft.description += ` [Additional: ${text}]`;

                setTimeout(() => {
                    addMessageToChat(`
                        <strong>Updated based on your input:</strong><br>
                        <ul>
                            <li><strong>Category:</strong> ${APP_STATE.currentCategory}</li>
                            <li><strong>Location:</strong> ${APP_STATE.complaintDraft.location}</li>
                            <li><strong>Summary:</strong> ${verification.summary || 'Details recorded'}</li>
                            <li><strong>Severity:</strong> <span style="color: ${verification.severity === 'High' ? '#ef4444' : verification.severity === 'Medium' ? '#f59e0b' : '#22c55e'}">${verification.severity}</span></li>
                        </ul>
                        <br>Type <strong>"Yes"</strong> or <strong>"Confirm"</strong> to submit. Type <strong>"No"</strong> to cancel.
                    `, 'bot');
                }, 500);
            ...
            if (lower.includes('yes') || lower.includes('confirm') || lower.includes('ok')) {
                ...
                try {
                    ...
                    console.error("Firestore Error (Fallback):", dbError);
                    showToast('Error saving complaint (Backup)', 'error');
                }
            } else {
                setTimeout(() => {
                    addMessageToChat(`Complaint cancelled. You can start over by selecting a category.`, 'bot');
